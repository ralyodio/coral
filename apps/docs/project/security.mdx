...
`[credentials].encryption_key_env` and
`[credentials].decryption_key_envs`. Coral reads them once at startup. The
active key encrypts new documents; previous keys are decrypt-only so existing
documents remain readable during rotation. See
[Configuration](/reference/configuration#credentials) for the format and
rotation behavior.
