...
The `[credentials]` section configures two separate storage paths:

- `storage` selects where source secrets are stored. See the
  [security model](/project/security#source-secret-storage).
- `encryption_key_env` names the environment variable containing the active
  key-encryption key (KEK) for identity setup inputs.
- `decryption_key_envs` lists environment variables containing previous KEKs
  that may decrypt existing identity setup inputs but are never used for new
  writes.

```toml
[credentials]
storage = "auto"
encryption_key_env = "CORAL_IDENTITY_KEK_CURRENT"
decryption_key_envs = ["CORAL_IDENTITY_KEK_PREVIOUS"]
```
...
With SQLite, Coral uses `credentials/encryption.key` in the local state
