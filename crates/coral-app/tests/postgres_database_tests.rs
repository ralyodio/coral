...
async fn server_lifecycle_can_start_with_postgres_database_config() {
    ...
    let server = start_server_with_postgres_config().await;
...
async fn start_server_with_postgres_config() -> PostgresServer {
...
async fn assert_workspace_service_round_trips_against_postgres(app: &AppClient) {
    ...
    let initial_workspaces = workspace_client
        .list_workspaces(Request::new(ListWorkspacesRequest {}))
        .await
        .expect("list initial workspaces")
        .into_inner()
        .workspaces;
    assert!(
        initial_workspaces
        ...
        "Postgres-backed server should expose the default workspace"
    );
