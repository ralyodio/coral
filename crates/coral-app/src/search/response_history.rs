...
use crate::state::db::{
    CoralDb, TraceSearchResponseCapture, TraceSearchResponseInsertResult, now_unix_nanos_i64,
};
...
impl SearchResponseHistory {
    ...
    pub(crate) fn capture(
        ...
        let (response_proto, oversized_bytes) = if encoded_len <= SEARCH_RESPONSE_HISTORY_MAX_BYTES
        {
            (Some(response.encode_to_vec()), None)
        } else {
            (None, Some(i64::try_from(encoded_len).unwrap_or(i64::MAX)))
        };
...
impl SearchResponseHistoryWorkerState {
    ...
    async fn retain(&mut self, capture: TraceSearchResponseCapture) {
...
async fn prune_search_response_history(db: &CoralDb, retention: Duration) -> bool {
    ...
    let cutoff_unix_nanos = retention_cutoff_unix_nanos(now_unix_nanos, retention);
    let prune = db.prune_trace_search_responses_before(cutoff_unix_nanos);
    tokio::pin!(prune);
    ...
    match result {
        Ok(_deleted) => true,
        Err(error) => {
            tracing::warn!(error = ?error, "failed to prune expired Search response history");
            false
        }
    }
}

pub(crate) fn retention_cutoff_unix_nanos(now_unix_nanos: i64, retention: Duration) -> i64 {
    let retention_nanos = i128::try_from(retention.as_nanos()).unwrap_or(i128::MAX);
    let cutoff_unix_nanos = i128::from(now_unix_nanos).saturating_sub(retention_nanos);
    i64::try_from(cutoff_unix_nanos).unwrap_or(i64::MIN)
}
...
mod tests {
    ...
    use super::{
        ...
        retention_cutoff_unix_nanos,
    };
    use crate::search::result::SearchExecutionIdentity;
    use crate::state::AppStateLayout;
    use crate::state::db::{
        CoralDb, DatabaseConfig, DbRepos, ResolvedDatabaseConfig, TraceSearchResponseCapture,
        TraceSearchResponseInsertResult, TraceSearchResponseRecord,
    };

    #[test]
    fn retention_cutoff_saturates_at_the_timestamp_bounds() {
        assert_eq!(retention_cutoff_unix_nanos(10, Duration::from_nanos(4)), 6);
        assert_eq!(
            retention_cutoff_unix_nanos(10, Duration::from_secs(u64::MAX)),
            i64::MIN
        );
    }
    ...
    async fn one_mebibyte_boundary_is_measured_before_encoding_and_stored_complete() {
        ...
        let (history, worker) = start_history(&db, true, test_retention());
        history.flush().await;
        ...
        assert!(history.capture("alpha", Some(&boundary_identity), &response));
        history.flush().await;
        let record = get_record(&db, &boundary_identity)
            .await
            .expect("captured boundary response");
        assert_eq!(
            record.response_proto.as_ref().map(Vec::len),
            Some(SEARCH_RESPONSE_HISTORY_MAX_BYTES)
        );
        assert_eq!(record.oversized_bytes, None);
    ...
    async fn startup_pruning_runs_independently_of_capture_enablement() {
        ...
        let (history, worker) = start_history(&db, false, Duration::ZERO);
        history.flush().await;
        ...
        assert!(!history.capture("alpha", Some(&disabled_identity), &empty_response()));
        history.flush().await;
    ...
    async fn capture_prunes_opportunistically_at_most_once_per_hour() {
        ...
        let (history, worker) = start_history(&db, true, Duration::from_hours(1));
        history.flush().await;
        ...
        assert!(history.capture("alpha", Some(&second_fresh), &empty_response()));
        history.flush().await;
    ...
    async fn complete_grouped_response_and_zero_result_response_round_trip() {
        ...
        assert!(history.capture("alpha", Some(&complete_identity), &response));
        history.flush().await;

        let record = get_record(&db, &complete_identity)
            .await
            .expect("captured response");
        let encoded = record.response_proto.expect("complete protobuf");
        assert_eq!(
        ...
        assert!(history.capture("alpha", Some(&zero_identity), &zero_response));
        history.flush().await;
        let zero_record = get_record(&db, &zero_identity)
            .await
    ...
    async fn disabled_capture_is_absent() {
        ...
        let (history, worker) = start_history(&db, false, test_retention());
        history.flush().await;
        assert!(!history.capture("alpha", Some(&disabled_identity), &empty_response()));
        assert_eq!(get_record(&db, &disabled_identity).await, None);
        shutdown_history(history, worker).await;
    ...
    async fn oversized_response_stores_only_its_original_encoded_size() {
        ...
        assert!(history.capture("alpha", Some(&identity), &response));
        history.flush().await;

        assert_eq!(
            get_record(&db, &identity).await,
            Some(TraceSearchResponseRecord {
                response_proto: None,
                oversized_bytes: Some(i64::try_from(encoded_len).expect("encoded size fits i64")),
            })
        );
        shutdown_history(history, worker).await;
    }
    ...
    async fn full_queue_drops_capture_without_waiting_for_the_worker() {
        ...
        for index in 0..SEARCH_RESPONSE_HISTORY_QUEUE_CAPACITY {
            ...
            assert!(history.capture("alpha", Some(&queued_identity), &empty_response()));
        }
        let dropped_identity = identity("trace-dropped", "span-dropped");
        assert!(!history.capture("alpha", Some(&dropped_identity), &empty_response()));
    ...
    async fn database_contention_does_not_block_capture_and_the_worker_eventually_persists() {
        ...
        assert!(history.capture("alpha", Some(&captured_identity), &empty_response()));
    ...
    fn start_history(
        ...
        enabled: bool,
        retention: Duration,
    ) -> (SearchResponseHistory, SearchResponseHistoryWorker) {
        SearchResponseHistory::start(Arc::clone(db), enabled, retention)
    }
    ...
    ) -> Option<TraceSearchResponseRecord> {
        db.get_trace_search_response("alpha", &identity.trace_id, &identity.span_id, i64::MIN)
            .await
