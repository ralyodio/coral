...
use crate::telemetry::local_store::{
    ...
    TraceSpanRecord, TraceStore, TraceStoreError, TraceSummaryRecord,
};
        ...
        instrument_grpc(span, async move {
            ...
            let page = match (view, db) {
                (TraceListView::All, Some(db)) => {
                    ...
                    let workspace_name =
                        workspace.map(|workspace| workspace.as_str().to_string());
                    let mut summaries = match workspace_name.as_deref() {
                        Some(workspace_name) => {
                            session
                                .trace_summaries()
                                .list_for_workspace(
