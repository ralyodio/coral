...
use super::{CoralDb, DbError, DbRepos, TraceSearchResponseRecord};
...
impl CoralDb {
    ...
    pub(crate) async fn prune_trace_search_responses_before(
        &self,
        retention_cutoff_unix_nanos: i64,
    ) -> Result<u64, DbError> {
...
mod tests {
    ...
    use super::{
        ...
        TraceSearchResponseMutationBarrier, TraceSearchResponsePruneBatchResult,
    };
    use crate::bootstrap;
    use crate::state::AppStateLayout;
    use crate::state::db::{
        CoralDb, DatabaseConfig, DbError, DbRepos, ResolvedDatabaseConfig,
        TraceSearchResponseRecord,
    };
    ...
    async fn trace_search_response_pruning_batches_workspace_mutations() {
        ...
        insert_response(&db, second_workspace_id, "expired", 1).await;

        assert_eq!(
            db.prune_trace_search_responses_before(33)
                .await
                .expect("prune expired response batches"),
            34
        );
        assert!(
            db.get_trace_search_response(workspace_id, "expired-32", "prune-span", i64::MIN)
                .await
                .expect("read final response after pruning multiple batches")
                .is_none()
        );
        assert!(
            db.get_trace_search_response(second_workspace_id, "expired", "prune-span", i64::MIN,)
                .await
                .expect("read response from second pruned workspace")
                .is_none()
        );
    }
    ...
    fn trace_search_response_debug_output_redacts_payloads() {
        ...
        let capture = capture("workspace", "trace", "span", 1, Some(payload.clone()), None);
        let record = TraceSearchResponseRecord {
            response_proto: Some(payload),
            oversized_bytes: None,
        };

        for debug_output in [format!("{capture:?}"), format!("{record:?}")] {
            assert!(!debug_output.contains("private-search-response"));
            assert!(!debug_output.contains(&raw_payload_debug));
            assert!(debug_output.contains("response_proto_bytes: Some(23)"));
        }
    }
    ...
    async fn assert_trace_search_response_repository_round_trip(db: &CoralDb, workspace_id: &str) {
        ...
        let trace_id = "shared-trace";
        let search_span_id = "shared-search-span";
        assert_eq!(
            db.insert_trace_search_response(capture(
                workspace_id,
                trace_id,
        ...
        assert_eq!(
            db.get_trace_search_response(workspace_id, trace_id, search_span_id, 10)
                .await
        ...
        assert_eq!(
            db.get_trace_search_response(workspace_id, trace_id, search_span_id, 11)
                .await
        ...
        assert_eq!(
            db.get_trace_search_response(workspace_id, trace_id, search_span_id, i64::MIN)
                .await
        ...
        assert_eq!(
            ...
            .await
            .expect("insert same trace identity in another workspace"),
            TraceSearchResponseInsertResult::Inserted
        );
        assert_eq!(
            db.get_trace_search_response(&second_workspace_id, trace_id, search_span_id, i64::MIN,)
                .await
                .expect("read workspace-isolated oversized response"),
            Some(TraceSearchResponseRecord {
                response_proto: None,
                oversized_bytes: Some(1_048_577),
            })
        );
            ...
            [("neither", None, None), ("both", Some(vec![7]), Some(7))]
        {
            ...
            .await
            .expect_err("exactly one stored outcome must be present");
        }
        ...
        for (trace_id, recorded_at_unix_nanos) in
            [("expired", i64::MIN), ("retained", i64::MIN + 1)]
        {
            assert_eq!(
                db.insert_trace_search_response(capture(
    ...
    async fn assert_capture_serializes_with_workspace_delete(db: &CoralDb, prefix: &str) {
        ...
        assert!(
            db.get_trace_search_response(&workspace_id, "race-trace", "race-span", i64::MIN,)
                .await
    ...
    async fn assert_prune_holds_workspace_before_deletion(
        ...
        insert_response(db, workspace_id, "prune-first", 1).await;
        let mutation_barrier = TraceSearchResponseMutationBarrier::new();
